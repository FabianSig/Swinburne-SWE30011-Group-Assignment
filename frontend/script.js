// Required MQTT module
const mqtt = require('mqtt');

// Adafruit IO credentials
const AIO_SERVER = 'io.adafruit.com';
const AIO_USERNAME = 'YOUR_USERNAME';
const AIO_KEY = 'YOUR_KEY';

// Feeds
const SENSOR_DATA_FEED = '/feeds/sensor-data';
const COMMANDS_FEED = '/feeds/commands';

// Connect to Adafruit IO MQTT broker
const client = mqtt.connect(`mqtt://${AIO_SERVER}`, {
  username: AIO_USERNAME,
  password: AIO_KEY,
});

client.on('connect', () => {
  client.subscribe(SENSOR_DATA_FEED);
  console.log('Connected and subscribed to Adafruit IO');
});

client.on('message', (topic, payload) => {
  if (topic === SENSOR_DATA_FEED) {
    const data = payload.toString().split(',');
    const parsedData = {
      light_levels: parseFloat(data[0].split(':')[1]),
      humidity: parseFloat(data[1].split(':')[1]),
      temperature: parseFloat(data[2].split(':')[1]),
      moisture_levels: parseFloat(data[3].split(':')[1]),
    };
    // Update charts with newData
    updateAllCharts(parsedData);
  }
});

function sendCommand(command) {
  client.publish(COMMANDS_FEED, command);
}
